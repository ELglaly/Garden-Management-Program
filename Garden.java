...
public class Garden {
	...
	public void plantGarden(int row, int col, String type) {
		if (row >= rows || col >= cols || land[row][col] != null) {
			...
			if (RunGarden.vegetables.contains(type)) {
				land[row][col] = new Vegetable(type, 0, "Red", 1, Health.Good, false);
			} else if (RunGarden.fruits.contains(type)) {
				land[row][col] = new Fruit(type, 0, "Red", 1, Health.Good, false, 0);

			} else if (RunGarden.flowers.contains(type)) {
				land[row][col] = new Flower(type, 0, "Red", 1, Health.Good, "0", false);

			} else if (RunGarden.trees.contains(type)) {
				land[row][col] = new Tree(type, 0, "Red", 1, Health.Good, 0, true, false);
			}
			 // Set the type of the newly planted plant
			try {
				land[row][col].setType(type);
			} catch (Exception ex) {
	...
	public void PrintGarden() {
		char arr[][] = new char[(rows) * 5][cols * 5];// 2D character array to represent the garden

		// Initialize the garden grid with empty spaces
		for (int i = 0; i < arr.length; i++) {
			for (int j = 0; j < arr[i].length; j++) {
				arr[i][j] = '.'; // Fill with '.' to represent empty spaces
			}
		}
